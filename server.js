  ...
  res.json({
    ...
    catalog: MODEL_CATALOG.map((m) => {
      ...
      return {
        ...
        // Each level carries the exact request it produces, so the card's tooltip
        // is generated by the same code that builds the call.
        thinkingOptions: { ...opts, options: opts.options.map((o) => ({ ...o, detail: thinkingProfile({ ...m, effort: o.value }).detail })) },
      };
    }),
    tasks: TASKS.map((t) => ({
      id: t.id,
